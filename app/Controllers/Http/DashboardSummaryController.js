...
class DashboardSummary {
  async index() {
    ...
    const employees = await User.query()
      ...
      .withCount('evaluationCycleDevelopmentPlans as employeeReceivedPdi')
      .orderBy('name', 'asc')
      .fetch();
  ...
  async show({ params }) {
    ...
    return User.query()
      ...
      .withCount('evaluationCycleAnswers as employeeEvaluationAnswers', (builder) => {
        builder.whereRaw('(user_finished = 0 or user_finished is null)').where({ evaluation_cycle_id: id });
      })
      .withCount('evaluationCycleJustificatives as employeeEvaluationJustificatives', (builder) => {
        builder.whereRaw('(user_finished = 0 or user_finished is null)').where({ evaluation_cycle_id: id });
      })
      .withCount('evaluationCycleAnswers as leaderEvaluationAnswers', (builder) => {
        builder.whereRaw('(leader_finished = 0 or leader_finished is null)').where({ evaluation_cycle_id: id });
      })
      .withCount('evaluationCycleJustificatives as leaderEvaluationJustificatives', (builder) => {
        builder.whereRaw('(leader_finished = 0 or leader_finished is null)').where({ evaluation_cycle_id: id });
      })
      ...
      .withCount('evaluationCycleComments as employeeConfirmFeedback', (builder) => {
        builder.whereRaw('employee_receipt_confirmation_date is null').where({ evaluation_cycle_id: id });
      })
