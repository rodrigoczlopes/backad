...
class EvaluationCycleAnswerSchema extends Schema {
  up() {
    this.table('evaluation_cycle_answers', (table) => {
      ...
      table.uuid('conciliator_id').unsigned().references('id').inTable('users').onDelete('SET NULL').onUpdate('CASCADE');
    });
  }
  ...
  down() {
    this.table('evaluation_cycle_answers', (table) => {
      ...
      table.dropColumn('conciliator_id').unsigned().references('id').inTable('users').onDelete('SET NULL').onUpdate('CASCADE');
    });
  }
}
